...
LANGS = {
    "python": {
        ...
        "src": "sol.py",
        "compile": None,
        "run": ["python", "sol.py"] if IS_WINDOWS else ["python3", "sol.py"],
    },
    "cpp": {
        "label": "C++17",
        "src": "sol.cpp",
        "compile": ["g++", "-O2", "-pipe", "-std=c++17", "-w", "-o", "sol", "sol.cpp"],
        "run": ["sol.exe"] if IS_WINDOWS else ["./sol"],
    },
    "java": {
        "label": "Java",
        "src": "Main.java",
...
def execute(run_cmd, input_data: str, time_limit_ms: int, mem_limit_mb: int, cwd: str,
    ...
    try:
        if IS_WINDOWS and run_cmd[0] == "sol.exe":
            run_cmd = [os.path.join(cwd, "sol.exe")] + run_cmd[1:]
